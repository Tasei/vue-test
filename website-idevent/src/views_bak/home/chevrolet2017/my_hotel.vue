...
<div>
    <div class="no-hotel" v-if="!hotelInfo">
        ...
        <router-link :to="{name:'home_index'}" class="btn btn-lg btn-primary">返回首页</router-link>
    </div>
    ...
    <div class="fd-hotel" v-else>
        ...
        <div class="hotel-info hotel-introduce" :class="{expanded: introduceExpanded}" v-if="hotelInfo && hotelInfo.introduce">
            ...
            <a v-if="!introduceExpanded" class="more text-primary" @click="introduceMore(true)" href="javascript:void(0);">查看更多</a>
        </div>
        ...
        <div class="hotel-info" v-if="detail && detail.hotel_details">
            ...
            <table class="table table-detail">
                ...
                <tr>
                    ...
                    <td><template v-if="hotelInfo">{{hotelInfo.name}}</template></td>
                </tr>
                <tr>
                    <td>酒店房型</td>
                    <td>
    ...
    <!--             <tr v-if="detail.hotel_details.is_share">
                    <td>员工编号</td>
            ...
            getDetail () {
                ...
                this.$http.post('order/detail').then((response) => {
                    ...
                    if (ret.err == 0) {
                        ...
                        //经销商
                        if (['18', '33', '38'].indexOf(this.user.group_id) > -1) {
                            this.$http.post('hotel/fordHotelDetail')
                            .then((response) => {
                                let ret = response.body
                                if (ret.err == 0) {
                            ...
                            self.$http.post('hotel/getById', {
                                id: self.detail.hotel_details.hotel_id
                            }).then((response) => {
                                let ret = response.body
                                if (ret.err == 0) {
                                    self.hotelInfo = ret.res
