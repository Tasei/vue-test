...
<div class="interact-wrapper">
	...
	<div class="interact-banners">
		...
		<div class="interact-banner interact-draw over" @click="gotoDraw">
			<div class="btn-wrapper">
				<button class="btn btn-outline">已结束</button>
			</div>
		</div>
	</div>
	</template>
...
export default {
	...
	methods: {
		gotoDraw(){
			...
			if (['18','33','38','57'].indexOf(this.user.group_id) < 0) {
				MessageBox.alert('仅经销商可参与抽奖, 感谢您的关注!')
				return
			}

			let self = this
			let voted = this.$cookie.get("FORD_VOTED_"+this.user.id)
			if (!voted) {
				MessageBox.alert('完成SUV家族互动问答即可参与抽奖').then(action => {
					//goto SUV
					self.gotoVote(self.groups[self.groups.length-1])
				});
				return
			}
			window.location.href="http://vip.sodiaopai.com/ford/phone/"
		},
		gotoVote(group) {
...
.interact-banner{
	...
	&:nth-child(1) {
		background-image: url("../../../assets/images/mobile/ford/interact-1.jpg");
	}
	&:nth-child(2) {
		background-image: url("../../../assets/images/mobile/ford/interact-2.jpg");
	}
	&:nth-child(3) {
		background-image: url("../../../assets/images/mobile/ford/interact-3.jpg");
	}

	&:last-child {
		background-image: url("../../../assets/images/mobile/ford/interact-4.jpg");
	}
